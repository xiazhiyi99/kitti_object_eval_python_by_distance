...
def clean_data_by_distance(gt_anno, dt_anno, current_class, difficulty):
    ...
    for i in range(num_gt):
        ...
        if DISTANCE_COVER:
            if( (gt_anno["occluded"][i] > MAX_OCCLUSION[2]) or (gt_anno["truncated"][i] > MAX_TRUNCATION[2])or (height <= MIN_HEIGHT[2]) or 
            ...
            if difficulty==0:
                ...
                if( (gt_anno["occluded"][i] > MAX_OCCLUSION[2]) or (gt_anno["truncated"][i] > MAX_TRUNCATION[2])or (height <= MIN_HEIGHT[2]) or 
                    (dis > MAX_DISTANCE[difficulty]) or (dis <= MAX_DISTANCE[difficulty - 1])
                    ):
                    # if gt_anno["difficulty"][i] > difficulty or gt_anno["difficulty"][i] == -1:
                    ignore = True
